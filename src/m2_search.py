...
def _get_qdrant_client():
    ...
    try:
        client = QdrantClient(host=QDRANT_HOST, port=QDRANT_PORT, timeout=1.5)
        client.get_collections()
        _shared_qdrant_client = client
    except Exception:
        storage_path = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "data", "qdrant_local")
...
class DenseSearch:
    ...
    def index(self, chunks: list[dict], collection: str = COLLECTION_NAME) -> None:
        ...
        dim = getattr(encoder, "get_sentence_embedding_dimension", lambda: EMBEDDING_DIM)() or EMBEDDING_DIM
...
def reciprocal_rank_fusion(results_list: list[list[SearchResult]], k: int = 60,
    ...
    for result_list in results_list:
        for rank, res in enumerate(result_list):
            ...
            if res.metadata:
                rrf_scores[res.text]["metadata"].update(res.metadata)
