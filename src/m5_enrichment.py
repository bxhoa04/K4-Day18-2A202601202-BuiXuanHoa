...
from config import OPENAI_API_KEY, GROQ_API_KEY, LLM_API_KEY, LLM_BASE_URL, LLM_MODEL
...
def _get_llm_client():
    ...
    if _cached_llm_client is None:
        ...
        if LLM_BASE_URL:
            _cached_llm_client = OpenAI(api_key=LLM_API_KEY, base_url=LLM_BASE_URL)
        else:
            _cached_llm_client = OpenAI(api_key=LLM_API_KEY)
    ...
    if LLM_API_KEY:
        try:
            ...
            resp = client.chat.completions.create(
                model=LLM_MODEL,
                messages=[
                    {"role": "system", "content": """Phân tích đoạn văn và trả về JSON hợp lệ (không kèm giải thích):
{
  ...
  "metadata": {"topic": "...", "entities": ["..."], "category": "policy|hr|it|finance", "language": "vi|en"}
    ...
    questions = [f"{s.rstrip('.')}?" for s in raw_sentences[:3]]
    context_line = f"Trích từ {source}. " if source else ""
    return {
        "summary": summary,
        "questions": questions,
